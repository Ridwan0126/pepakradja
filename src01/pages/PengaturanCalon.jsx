...
export default function PengaturanCalon() {
  ...
  const hasVotes = (calonId) => {
    const votes = storage.get("votes", []);
    return votes.some((vote) => vote.calonId === calonId);
  };
  ...
  const handleSubmit = (e) => {
    ...
    if (formData.alamatWakil.length > 200) {
      Swal.fire({
        icon: "error",
