...
export default function LaporanRekap() {
  ...
  const loadData = () => {
    ...
    setVotesOnline(savedVotesOnline)
    setVotesOffline(savedVotesOffline)

    generateRekap(savedWilayah, savedPemilih, savedVotesOnline, savedVotesOffline)
  }

  const generateRekap = (wilayah, pemilihData, vOnline, vOffline) => {
    const rekapData = wilayah.map((w) => {
      const pemilihWilayah = pemilihData.filter((p) => p.wilayahPemilih === w.wilayahPemilih)
      const jumlahPemilih = pemilihWilayah.length

      const vOnlineWilayah = vOnline.filter((v) => {
        const pemilih = pemilihData.find((p) => p.id === v.pemilihId)
        return pemilih?.wilayahPemilih === w.wilayahPemilih
      })

      const vOfflineWilayah = vOffline.filter((v) => v.wilayahPemilih === w.wilayahPemilih)

      const onlineSah = vOnlineWilayah.filter((v) => v.calonId !== null).length
      const onlineTidakSah = vOnlineWilayah.filter((v) => v.calonId === null).length
      const offlineSah = vOfflineWilayah.filter((v) => v.statusSuara === "SAH").length
      const offlineTidakSah = vOfflineWilayah.filter((v) => v.statusSuara === "TIDAK SAH").length

      const suaraMasuk = onlineSah + onlineTidakSah + offlineSah + offlineTidakSah
  ...
  const exportPDF = () => {
    ...
    try {
      ...
      /* ================= SAVE ================= */
      const fileName = `Laporan-Rekap-${new Date().toISOString().split("T")[0]}.pdf`
  ...
  const exportExcel = () => {
    ...
    link.setAttribute("download", `Laporan-Rekap-${new Date().toISOString().split("T")[0]}.csv`)
    link.click()
      ...
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }}>
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl md:text-3xl font-bold text-[var(--color-text-primary)]">Laporan Rekap Suara</h1>
